  ...
  if (msg->bus == 0U) {
    ...
    if (addr == MSG_ESC_51) {
      ...
      if (GET_LEN(msg) >= 16) {
        ...
        uint32_t fr = ((uint32_t)data_ptr[10] | ((uint32_t)data_ptr[11] << 8));
        uint32_t rl = ((uint32_t)data_ptr[12] | ((uint32_t)data_ptr[13] << 8));
        uint32_t rr = ((uint32_t)data_ptr[14] | ((uint32_t)data_ptr[15] << 8));
    ...
    if (addr == MSG_Motor_51) {
      ...
      // TSK_Status is at bit 88 (byte 11, bits 0-2) in 32-byte CAN-FD message
      int acc_status = 0;
      if (GET_LEN(msg) >= 12) {
        // Access byte 11 using pointer to avoid compiler array bounds warning
        // This is safe because we check GET_LEN(msg) >= 12 above
        const uint8_t *data_ptr = (const uint8_t *)msg->data;
        acc_status = (int)(data_ptr[11] & 0x07U);
      }
    ...
    if (addr == MSG_Motor_54) {
      // Motor_54 is a 32-byte CAN-FD message, Accelerator_Pressure is at byte 21
      if (GET_LEN(msg) >= 22) {
        const uint8_t *data_ptr = (const uint8_t *)msg->data;
        int accel_pedal_value = (int)data_ptr[21] - 37U;
